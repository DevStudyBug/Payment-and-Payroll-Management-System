...
public class OrgAdminConcernServiceImpl implements OrgAdminConcernService {
	...
	public Page<ConcernResponseDto> getAllConcerns(Long orgId, String status, String priority, int page, int size) {
		...
		List<ConcernResponseDto> dtoList = concerns.stream().map(this::mapToDto).collect(Collectors.toList());
	...
	@Override
	@Transactional(readOnly = true)
	public ConcernResponseDto getConcernByTicket(Long orgId, String ticketNumber) {
	...
	private ConcernResponseDto mapToDto(ConcernEntity c) {
		return ConcernResponseDto.builder().concernId(c.getConcernId()).ticketNumber(c.getTicketNumber())
				.category(c.getCategory()).priority(c.getPriority()).description(c.getDescription())
				.status(c.getStatus()).adminResponse(c.getAdminResponse()).attachmentUrl(c.getAttachmentUrl())
				.employeeName(
						c.getEmployee() != null ? c.getEmployee().getFirstName() + " " + c.getEmployee().getLastName()
								: null)
	...
	private void validateOrgAccess(ConcernEntity concern, Long orgId) {
		if (!concern.getOrganization().getOrgId().equals(orgId)) {
	...
	private void validateStatusTransition(ConcernEntity concern, String newStatus) {
		...
		if ((ConcernConstants.STATUS_RESOLVED.equalsIgnoreCase(current)
			...
			throw new IllegalStateException(
					"Resolved or rejected concerns cannot be modified further unless reopened by the employee.");
		}
